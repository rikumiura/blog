...
- TDDの各ステップでcommitを作成すること
- テストを先に書くことで、仕様を明確にしてから実装に入る
- 新機能の追加・バグ修正いずれの場合も、まずテストを作成してから実装を始めること
- テストダブル（インメモリリポジトリ等）を活用し、外部依存なしで高速にテストを回すこと
...
- 作業完了後は`/codex:review --wait`を使用してレビューし、修正すること
...
- 想定内エラー（バリデーション不正、業務ルール違反、未存在、重複など）は `Result`（判別共用体）で返す
- 想定外エラー（DB障害、外部API障害、I/O障害、バグなど）は `throw` して上位へ伝播させる
- HTTP 境界（Hono ルート）で `Result` を 4xx に変換し、想定外エラーは 5xx として処理する
- 判断基準は「呼び出し側がその場で回復・代替できるか」。できるなら `Result`、できないなら `throw`
- 想定外エラーを `Result` に混ぜて握りつぶさない（監視・障害分析・リトライ判断の精度低下を防ぐ）
